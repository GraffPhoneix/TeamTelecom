import img1 from './Img/News/img_1.png';

const dataBase = {
    "products": [
        {
            "body": "",
            "id": 1,
            "imgURL": "https://www.telecomarmenia.am/images/product/10/17522148205563/245x280c-center.jpeg",
            "price": "107,900 ֏",
            "status": "none",
            "title": "Планшет Huawei MatePad 11.5\" S (inbox keyboard) | 128 GB",
            "type": "#smartphone"
        },
        {
            "body": "",
            "id": 2,
            "imgURL": "https://www.telecomarmenia.am/images/product/10/1758792966104/245x280c-center.png",
            "price": "37,900 ֏",
            "status": "none",
            "title": "iPhone 17 Pro TechWoven Case with MagSafe",
            "type": "#accsesories"
        },
        {
            "body": "Начиная с 10,220 ֏/месяц",
            "id": 3,
            "imgURL": "https://www.telecomarmenia.am/images/product/10/17582143854401/245x280c-center.jpeg",
            "price": "619, 000 ֏",
            "status": "none",
            "title": "Apple iPhone 17 Pro | 256 GB",
            "type": "#smartphone"
        },
        {
            "body": "",
            "id": 4,
            "imgURL": "https://www.telecomarmenia.am/images/product/10/17524966359803/245x280c-center.jpeg",
            "price": "104, 900 ֏",
            "status": "none",
            "title": "Очиститель воздуха Xiaomi Smart Air Purifier 4 ",
            "type": "#device"
        },
        {
            "body": "",
            "id": 5,
            "imgURL": "https://www.telecomarmenia.am/images/product/11/17587931029429/245x280c-center.png",
            "price": "31,900 ֏",
            "status": "none",
            "title": "iPhone 17 Silicone Case with MagSafe",
            "type": "#accsesories"
        },
        {
            "body": "",
            "id": 6,
            "imgURL": "https://www.telecomarmenia.am/images/product/10/17524766853044/245x280c-center.png",
            "price": "157,900 ֏",
            "status": "none",
            "title": "Планшет Huawei MatePad 11.5\" S (inbox keyboard) | 256 GB",
            "type": "#smartphone"
        },
        {
            "body": "",
            "id": 7,
            "imgURL": "https://www.telecomarmenia.am/images/product/10/17524968477341/245x280c-center.jpeg",
            "price": "77, 900 ֏",
            "status": "none",
            "title": "Очиститель воздуха Xiaomi Mi Smart Air Purifier 4 Lite ",
            "type": "#device"
        },
        {
            "body": "Начиная с 11,170 ֏/месяц",
            "id": 8,
            "imgURL": "https://www.telecomarmenia.am/images/product/11/17582734427687/245x280c-center.jpeg",
            "price": "699,900 ֏",
            "status": "none",
            "title": "iPhone Air | 512 GB",
            "type": "#smartphone"
        },
        {
            "body": "",
            "id": 9,
            "imgURL": "https://www.telecomarmenia.am/images/product/10/17520444211666/245x280c-center.png",
            "price": "39,900 ֏",
            "status": "none",
            "title": "Смарт-часы Huawei Watch FIT3",
            "type": "#accsesories"
        },
        {
            "body": "",
            "id": 10,
            "imgURL": "https://www.telecomarmenia.am/images/product/10/17521393278161/245x280c-center.png",
            "price": "92,000 ֏",
            "status": "none",
            "title": "Пылесос  Xiaomi Robot Vacuum S20 (D106) BHR8629EU",
            "type": "#device"
        },
        {
            "body": "",
            "id": 11,
            "imgURL": "https://www.telecomarmenia.am/images/product/10/1752489837911/245x280c-center.png",
            "price": "529,900 ֏",
            "status": "none",
            "title": "Автоматическая кофемашина DELONGHI PRIMADONNA SOUL ECAM610.74.MB",
            "type": "#device"
        },
        {
            "body": "",
            "id": 12,
            "imgURL": "https://www.telecomarmenia.am/images/product/10/17524733335597/245x280c-center.png",
            "price": "77,900 ֏",
            "status": "none",
            "title": "Смарт-часы Huawei Watch GT5 41mm Gold Milanese Strap",
            "type": "#accsesories"
        },
        {
            "body": "",
            "id": 13,
            "imgURL": "https://www.telecomarmenia.am/images/product/10/17570544322496/245x280c-center.jpeg",
            "price": "49, 900 ֏",
            "status": "none",
            "title": "Планшет Oscal Pad 60 Kids 10.1 | 128 GB",
            "type": "#smartphone"
        },
        {
            "body": "",
            "id": 14,
            "imgURL": "https://www.telecomarmenia.am/images/product/10/17524788824632/245x280c-center.jpeg",
            "price": "215,900 ֏",
            "status": "none",
            "title": "Акустическая система JBL Authentics 300",
            "type": "#accsesories"
        },
        {
            "body": "",
            "id": 15,
            "imgURL": "https://www.telecomarmenia.am/images/product/10/17524960690751/245x280c-center.jpeg",
            "price": "239,900 ֏",
            "status": "none",
            "title": "Увлажнитель воздуха Dyson AM10",
            "type": "#device"
        }
    ],
    smartphones: {
        unlimited: [
            { header: 'Video Unlimit', title: 'Подключи и наслаждайся безлимитными видео!' },
            { header: 'Music Unlimit', title: 'Подключи и наслаждайся безлимитной музыкой' },
            { header: 'Social Unlimit', title: 'Подключи и наслаждайся безлимитными социальными сетями' },
            { header: 'Messenger Unlimit', title: 'Подключи и наслаждайся безлимитными мессенджерами' }
        ],
        giga: [
            { header: 'Giga 1+1ГБ', title: 'Высокоскоростной интернет-пакет для смартфона' },
            { header: 'Giga 2+1ГБ', title: 'Высокоскоростной интернет-пакет для смартфона' },
            { header: 'Giga 5+1ГБ', title: 'Высокоскоростной интернет-пакет для смартфона' },
            { header: 'Giga 10 ГБ', title: 'Высокоскоростной интернет-пакет для смартфона' }
        ],
        mega: [
            { header: 'Mega 300+', title: 'Ежедневный Интернет для смартфона: баланс больше не обнуляется.' },
            { header: 'Mega 500+', title: 'Ежедневный Интернет для смартфона: баланс больше не обнуляется.' },
            { header: 'Mega 300', title: 'Ежедневный Интернет для смартфона.' },
            { header: 'Mega 500', title: 'Ежедневный Интернет для смартфона.' }
        ]
    },
    payment: [
        { header: 'Баланс трансфер', title: 'Услуга позволяет абонентам предоплатной системы расчетов мобильной сети Team переводить деньги со своего счета на счет другого абонента…' },
        { header: 'Позвони мне', title: 'Услуга дает возможность абоненту предоплатой и постоплатной системы расчетов Team отправлять запрос с просьбой перезвонить ему. Абонент…' },
        { header: 'Доверительный платеж', title: 'Если ваш баланс близок к нулю, а возможности пополнить счет на данный момент нет, воспользуйтесь «Доверительным платежом».' },
        { header: 'Пополни мой счет', title: 'Услуга «Пополни мой счет» дает возможность мобильному абоненту предоплатой и постоплатной системы расчетов Team отправлять…' },
    ],
    roaming: [
        { header: 'Условия подключения роуминга', title: `                                       .` },
        { header: 'Тарифы роуминга на море и в воздухе', title: 'Будьте онлайн, даже на море и в воздухе' }
    ],
    isLogined: false,
    shop_pseudo_header_images: ['https://www.telecomarmenia.am/images/advanced_slider/2/17591324480321.png', 'https://www.telecomarmenia.am/images/advanced_slider/2/17591321353964.png', 'https://www.telecomarmenia.am/images/advanced_slider/2/17591321199707.png', 'https://www.telecomarmenia.am/images/advanced_slider/2/17591321075712.png']
};

if (JSON.parse(localStorage.getItem('dataBase')) === null) { localStorage.setItem('dataBase', JSON.stringify(dataBase)) }
export default JSON.parse(localStorage.getItem('dataBase'));